...
## and are created in the simulation are named differently, to make sure the
...
#number.answeroptions = rep(0 ,number.items) #Every item has 3 answer options
number.answeroptions = sample(c(0,4:6), number.items, replace=TRUE)
#Give each item a random max. of answer options.

# Define categories
#category = sample(1:3, number.items, replace = TRUE, prob = c(.5,.4,.1))
category = ifelse(number.answeroptions == 0, 1, 2)
