...
CumHist <- function(student.scores, n.item) {
  ...
  if (!is.na(mis[1])) {
    for (i in 1:length(mis)) {
      df[ (nrow(df) + 1), ] <- c(mis[i], 0, 0) #  add row with missing total score
    }
  ...
  g <- ggplot(data = df, aes(x = score, y = total)) +
    ...
    scale_x_continuous(limits = c(df[1, 1] - 1, n.item), breaks = 0:n.item,
                       expand = c(0, 0)) +
    theme_bw() +
...
if (!length(categories) == 0) {
  ...
  for (i in 1:length(categories)) {
    list2env(categories[[i]], globalenv())
    ...
    cat("\n\n")
    CumHist(student.scores, n.item = length(items))
  }
}
```